...
import org.apache.kafka.common.requests.{ExpireDelegationTokenRequest, ExpireDelegationTokenResponse, RequestContext, RequestHeader}
...
/**
 * Jazzer fuzz tests for `KafkaApis.handleExpireTokenRequest` / `handleExpireTokenRequestZk`.
 *
 * Covers `allowTokenRequests` rejection on `PLAINTEXT`, successful and failing
 * `DelegationTokenManager#expireToken` callbacks, ZooKeeper migration with an inactive
 * controller (`NOT_CONTROLLER`), client throttling, and forwarded inner requests.
 * `ExpireDelegationTokenResponse` exposes only error code, expiry timestamp, and throttle
 * time; there is no separate error message field.
 */
class HandleExpireTokenRequestFuzzTest extends KafkaApisTest {
  ...
  private def buildExpireRequestWithPlaintextSecurityProtocol(
      built: ExpireDelegationTokenRequest
  ...
  private def stubDelegationTokenManagerExpireSuccess(
    delegationTokenManager: DelegationTokenManager,
    resultingExpiryTimestampMs: Long
  ): Unit = {
    when(delegationTokenManager.expireToken(any(), any(), anyLong(), any()))
      .thenAnswer(invocation => {
        val callback = invocation.getArgument(3).asInstanceOf[(Errors, Long) => Unit]
        callback.apply(Errors.NONE, resultingExpiryTimestampMs)
      })
  }

  private def stubDelegationTokenManagerExpireError(
    delegationTokenManager: DelegationTokenManager,
    error: Errors,
  ...
  def fuzzTestExpireTokenRequestNotAllowedPlaintext(data: FuzzedDataProvider): Unit = {
    val requestWireVersion = data.consumeShort(
      ApiKeys.EXPIRE_DELEGATION_TOKEN.oldestVersion(),
      ApiKeys.EXPIRE_DELEGATION_TOKEN.latestVersion())
    val hmacLength = data.consumeInt(0, 128)
    val hmacBytes = data.consumeBytes(hmacLength).clone()
    val expiryTimePeriodMs = data.consumeLong(0L, 86_400_000L)

    resetHarness()
    stubNoThrottle()

    val requestData = new ExpireDelegationTokenRequestData()
      .setHmac(hmacBytes)
      .setExpiryTimePeriodMs(expiryTimePeriodMs)
    val built = new ExpireDelegationTokenRequest.Builder(requestData).build(requestWireVersion)
    val request = buildExpireRequestWithPlaintextSecurityProtocol(built)
    ...
    try {
      ...
      val response = verifyNoThrottling[ExpireDelegationTokenResponse](request)
  ...
  @FuzzTest(maxDuration = FUZZ_DURATION)
  def fuzzTestExpireTokenRequestZkDelegationTokenManagerSuccess(data: FuzzedDataProvider): Unit = {
    val requestWireVersion = data.consumeShort(
      ApiKeys.EXPIRE_DELEGATION_TOKEN.oldestVersion(),
      ApiKeys.EXPIRE_DELEGATION_TOKEN.latestVersion())
    val hmacLength = data.consumeInt(0, 128)
    val hmacBytes = data.consumeBytes(hmacLength).clone()
    val expiryTimePeriodMs = data.consumeLong(0L, 86_400_000L)
    val resultingExpiryTimestampMs = data.consumeLong(1L, Long.MaxValue / 4)
    ...
    val delegationTokenManager = mock(classOf[DelegationTokenManager])
    stubDelegationTokenManagerExpireSuccess(delegationTokenManager, resultingExpiryTimestampMs)
    val kafkaApis = createKafkaApis(tokenManager = delegationTokenManager)
    try {
      kafkaApis.handleExpireTokenRequest(request)
      val response = verifyNoThrottling[ExpireDelegationTokenResponse](request)
      assertEquals(Errors.NONE, response.error())
      assertEquals(resultingExpiryTimestampMs, response.data.expiryTimestampMs)
    } finally kafkaApis.close()
  }

  @FuzzTest(maxDuration = FUZZ_DURATION)
  def fuzzTestExpireTokenRequestZkDelegationTokenManagerReturnsError(data: FuzzedDataProvider): Unit = {
    val requestWireVersion = data.consumeShort(
      ApiKeys.EXPIRE_DELEGATION_TOKEN.oldestVersion(),
      ApiKeys.EXPIRE_DELEGATION_TOKEN.latestVersion())
    val hmacLength = data.consumeInt(0, 128)
    val hmacBytes = data.consumeBytes(hmacLength).clone()
    val expiryTimePeriodMs = data.consumeLong(0L, 86_400_000L)
    val useNotFoundError = data.consumeBoolean()
    val managerError =
      if (useNotFoundError) Errors.DELEGATION_TOKEN_NOT_FOUND
      else Errors.DELEGATION_TOKEN_EXPIRED
    val errorExpiryTimestampMs =
      if (useNotFoundError) data.consumeLong(0L, Long.MaxValue / 8)
      else DelegationTokenManager.ErrorTimestamp
    ...
    val delegationTokenManager = mock(classOf[DelegationTokenManager])
    stubDelegationTokenManagerExpireError(delegationTokenManager, managerError, errorExpiryTimestampMs)
    val kafkaApis = createKafkaApis(tokenManager = delegationTokenManager)
    try {
      kafkaApis.handleExpireTokenRequest(request)
      val response = verifyNoThrottling[ExpireDelegationTokenResponse](request)
  ...
  @FuzzTest(maxDuration = FUZZ_DURATION)
  def fuzzTestExpireTokenRequestZkMigrationInactiveController(data: FuzzedDataProvider): Unit = {
    val requestWireVersion = data.consumeShort(
      ApiKeys.EXPIRE_DELEGATION_TOKEN.oldestVersion(),
      ApiKeys.EXPIRE_DELEGATION_TOKEN.latestVersion())
    val hmacLength = data.consumeInt(0, 128)
  ...
  @FuzzTest(maxDuration = FUZZ_DURATION)
  def fuzzTestExpireTokenRequestThrottledResponse(data: FuzzedDataProvider): Unit = {
    val requestWireVersion = data.consumeShort(
      ApiKeys.EXPIRE_DELEGATION_TOKEN.oldestVersion(),
      ApiKeys.EXPIRE_DELEGATION_TOKEN.latestVersion())
    ...
    val delegationTokenManager = mock(classOf[DelegationTokenManager])
    stubDelegationTokenManagerExpireSuccess(delegationTokenManager, 7_777_666_555L)
    val kafkaApis = createKafkaApis(tokenManager = delegationTokenManager)
    try kafkaApis.handleExpireTokenRequest(request)
    finally kafkaApis.close()
  }

  @FuzzTest(maxDuration = FUZZ_DURATION)
  def fuzzTestExpireTokenRequestForwardedInnerRequest(data: FuzzedDataProvider): Unit = {
    val requestWireVersion = data.consumeShort(
      ApiKeys.EXPIRE_DELEGATION_TOKEN.oldestVersion(),
      ApiKeys.EXPIRE_DELEGATION_TOKEN.latestVersion())
    ...
    val delegationTokenManager = mock(classOf[DelegationTokenManager])
    stubDelegationTokenManagerExpireSuccess(delegationTokenManager, 100L)
    val kafkaApis = createKafkaApis(tokenManager = delegationTokenManager)
    try kafkaApis.handleExpireTokenRequest(request)
    finally kafkaApis.close()
  }
